...
Public Class Form_KodifBahanBaku
    ...
    Private Sub cmdSimpan_Click(sender As Object, e As EventArgs) Handles cmdSimpan.Click
        ...
        If LAdd Then
            ...
            SQL = "Select KodeBahan, NamaIndonesia 
                 From M_KodeBahan 
                Where KodeBahan = '" & KodeBahan.Text & "'
                  And aktifYN='Y' "
            dbTable = Proses.ExecuteQuery(SQL)
            If dbTable.Rows.Count <> 0 Then
                KodeBahan.Focus()
                MsgBox("Kode " & KodeBahan.Text & " SUDAH digunakan " & dbTable.Rows(0) !namaindonesia, vbCritical, "Warning!")
                Exit Sub
            End If

            SQL = "INSERT INTO m_KodeBahan (KodeBahan, NamaIndonesia, " &
                "NamaInggris, HSNo, AktifYN, UserID, LastUPD, TransferYN) VALUES " &
                "('" & KodeBahan.Text & "', '" & NamaIndonesia.Text & "', " &
                "'" & NamaInggris.Text & "', '" & HSNo.Text & "', 'Y', " &
                "'" & UserID & "',  GetDate(), 'N') "
            Proses.ExecuteNonQuery(SQL)
        ElseIf LEdit Then
            SQL = "Update m_KodeBahan SET " &
    ...
    Sub Data_Record()
        ...
        SQL = "SELECT * FROM m_KodeBahan WHERE aktifyn = 'Y' " &
    ...
    Sub Isi_Data()
        SQL = "SELECT * FROM m_KodeBahan " &
            "WHERE KodeBahan = '" & KodeBahan.Text & "' " &
            " AND aktifYN='Y' "
        dbTable = Proses.ExecuteQuery(SQL)
        If dbTable.Rows.Count <> 0 Then
            KodeBahan.Text = dbTable.Rows(0) !KodeBahan
            NamaIndonesia.Text = dbTable.Rows(0) !NamaIndonesia
            NamaInggris.Text = dbTable.Rows(0) !NamaInggris
            HSNo.Text = dbTable.Rows(0) !hsno
        End If
    End Sub
    ...
    Private Sub Cetak()
        ...
        Try
            ...
            objRep = New Rpt_KodifBahanBaku
            objRep.SetDataSource(dttable)
            Form_CetakLaporan.Text = "Daftar Bahan Baku"
    ...
    Private Sub DGView_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGView.CellClick
        If e.RowIndex >= 0 Then
            KodeBahan.Text = DGView.Rows(e.RowIndex).Cells(0).Value
            NamaIndonesia.Text = DGView.Rows(e.RowIndex).Cells(1).Value
            NamaInggris.Text = DGView.Rows(e.RowIndex).Cells(2).Value
            HSNo.Text = DGView.Rows(e.RowIndex).Cells(3).Value
        End If
        If e.ColumnIndex = 4 Then 'Edit
            tEdit = Proses.UserAksesTombol(UserID, "21_KODIF_BAHAN_BAKU", "edit")
            If tEdit Then
                If Trim(KodeBahan.Text) = "" Then
                ...
                LEdit = True
                AturTombol(False)
                KodeBahan.ReadOnly = True
                NamaIndonesia.Focus()
            Else
                MsgBox("Anda tidak berhak untuk edit data ini !", vbCritical + vbOKOnly, ".:Warning !")
                Exit Sub
            End If
        ElseIf e.ColumnIndex = 5 Then 'Hapus
            tHapus = Proses.UserAksesTombol(UserID, "21_KODIF_BAHAN_BAKU", "hapus")
            If tHapus Then
                If Trim(KodeBahan.Text) = "" Then
                    MsgBox("Kode Bahan Belum di pilih!", vbCritical, ".:ERROR!")
                    DGView.Focus()
                ...
                If MsgBox("Yakin hapus data " & Trim(DGView.Rows(DGView.CurrentCell.RowIndex).Cells(1).Value) & "?", vbYesNo + vbInformation, "Confirm!") = vbYes Then
                    SQL = "Update M_KodeBahan Set AktifYN = 'N', UserID = '" & UserID & "', LastUPD = GetDate(), TransferYN='N' " &
                        "Where KodeBahan = '" & KodeBahan.Text & "' "
                    Proses.ExecuteNonQuery(SQL)
                    ClearTextBoxes()
                    Data_Record()
                End If
            Else
                MsgBox("Anda tidak berhak untuk hapus data ini !", vbCritical + vbOKOnly, ".:Warning !")
                Exit Sub
            End If
        End If
    ...
    Private Sub Form_KodifBahanBaku_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        tTambah = Proses.UserAksesTombol(UserID, "21_KODIF_BAHAN_BAKU", "baru")
        tEdit = Proses.UserAksesTombol(UserID, "21_KODIF_BAHAN_BAKU", "edit")
        tHapus = Proses.UserAksesTombol(UserID, "21_KODIF_BAHAN_BAKU", "hapus")
        tLaporan = Proses.UserAksesTombol(UserID, "21_KODIF_BAHAN_BAKU", "laporan")

        AturTombol(True)
    End Sub

    Private Sub KodeBahan_KeyPress(sender As Object, e As KeyPressEventArgs) Handles KodeBahan.KeyPress
    ...
    Private Sub NamaInggris_KeyPress(sender As Object, e As KeyPressEventArgs) Handles NamaInggris.KeyPress
        If e.KeyChar = Chr(39) Then e.KeyChar = Chr(96)
        If e.KeyChar = Chr(13) Then HSNo.Focus()
    End Sub

    Private Sub HSNo_KeyPress(sender As Object, e As KeyPressEventArgs) Handles HSNo.KeyPress
        If e.KeyChar = Chr(39) Then e.KeyChar = Chr(96)
