...
Public Class Rpt_LabaRugi
    ...
    Public Sub HitungLabaRugi(IdRPT As String, Tanggal As Date)
        Dim SaldoAwal As Double, tSaldoAwal As Double = 0,
            Debet As Double, Kredit As Double, SaldoAkhir As Double, Spasi As Integer
        ...
        Dim MsgSQL As String, NoUrut As Integer
        Dim Mode As String = ""

        Mode = Microsoft.VisualBasic.Right(Trim(IdRPT), 5)
        Periode1 = Format(DateAdd("m", -1, Tanggal), "MM-yyyy")
        Periode2 = Format(Tanggal, "MM-yyyy")
        NoUrut = 1
        ...
        For a = 0 To RS05.Rows.Count - 1
            ...
            Else
                ...
                Else
                    ...
                    If Mode = "CLOSE" Then
                        MsgSQL = "Select isnull(Sum(Debet),0) as Debet From t_Jurnal " &
                            "Where AktifYN = 'Y' " &
                            "  And Convert(char(6), tanggal,112) = '" & Format(Tanggal, "yyyyMM") & "' " &
                        ...
                        MsgSQL = "Select isnull(Sum(Kredit),0) as Kredit From t_Jurnal " &
                            "Where AktifYN = 'Y' " &
                            "  And Convert(char(8), tanggal,112) = '" & Format(Tanggal, "yyyyMM") & "' " &
                            "  And AccountCode like '" & Trim(RS05.Rows(a) !KodeGL) & "%' "
                ...
                If RS05.Rows(a) !tUrut = "0700" Or RS05.Rows(a) !Urut = "200" Then
                    SaldoAkhir = tSaldoAwal + Kredit - Debet
                ElseIf RS05.Rows(a) !Urut = "302" Then
                    SaldoAkhir = tSaldoAwal + Debet - Kredit
                Else
                    SaldoAkhir = tSaldoAwal + Debet - Kredit
                End If
    ...
    Private Sub cmdCetak_Click(sender As Object, e As EventArgs) Handles cmdCetak.Click
        ...
        Dim IDRpt As String = "", xPeriode As String = "", MsgSQL As String

        Randomize()
        Me.Cursor = Cursors.WaitCursor
        cmdCetak.Enabled = False
        IDRpt = Microsoft.VisualBasic.Left(Replace(Trim(Rnd(10000) * 100000), ".", UserID) & "RPTLR", 30)

        HitungLabaRugi(IDRpt, Tgl1.Value)
